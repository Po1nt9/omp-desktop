fn main() {
    ...
    println!("cargo:rerun-if-env-changed=OMP_DESKTOP_UPDATER_PUBLIC_KEY");
    println!("cargo:rerun-if-env-changed=OMP_DESKTOP_UPDATER_ENDPOINT");
    println!("cargo:rustc-check-cfg=cfg(omp_desktop_updater_enabled)");
    ...
    // never registered.
    let updater_public_key = std::env::var("OMP_DESKTOP_UPDATER_PUBLIC_KEY")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty());
    let updater_endpoint = std::env::var("OMP_DESKTOP_UPDATER_ENDPOINT")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty());

    if let (Some(_key), Some(endpoint)) = (updater_public_key, updater_endpoint) {
        println!("cargo:rustc-cfg=omp_desktop_updater_enabled");
        // Expose non-secret endpoint URL to `option_env!` / status DTO.
        println!("cargo:rustc-env=OMP_DESKTOP_UPDATER_ENDPOINT={endpoint}");
    }
