...
2. **`docs/release/`** — 1.0 release authority: [acceptance matrix](docs/release/1.0-acceptance-matrix.md), [security audit checklist](docs/release/security-audit-checklist.md), [test coverage audit](docs/release/test-coverage-audit.md). Plan 10 execution closed 2026-07-31 at 61 PASS / 12 PARTIAL / 84 BLOCKED (the table's FAIL=1 is the grep self-count of the counts row — real FAIL=0). Remaining BLOCKED/PARTIAL items need Windows x64 or macOS Intel hardware, IM channel accounts, or an external security auditor; the owner waived pursuing them (recorded in the matrix header).
