...
- **Test 7 deviation:** implemented against a process-wide capture layer (`test_capture::global_events()`, `traced_only` filter) instead of a scoped `set_default`. Root cause: tracing's per-callsite interest cache is process-global; "remote_im: handle begin" is a callsite shared with other engine tests, and concurrent no-subscriber emissions race a scoped install to bake `Interest::never()` (observed: passes solo, fails ~80% under `remote_im`-module parallelism; bisect showed ANY concurrent test triggers it — not a specific interferer). Global default participates in the interest union deterministically; the `traced_only` filter keeps foreign no-span events out. Mechanism tests 1–5 and wiring test 6 keep scoped capture (unique callsites, no race). Verified: remote_im 4× + full suite 2× green.
