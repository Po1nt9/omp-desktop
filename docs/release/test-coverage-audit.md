...
|---|---|---|---|
| Frontend (vitest) | 95 files / 843 tests (843 pass) | `pnpm test` | React components, Tauri command mocks, fail-closed behavior, UI logic, hooks, i18n rendering, code language detection, end-of-turn detection, project path handling, voice audio, redaction, runtime availability, file preview. +5 AC-1.5 transport-seam round-trip tests (todo.list five-state, subagents.status/setEnabled, rejection→runtime_unavailable, fail-closed without transport) (2026-07-31 evening); +3 AC-10.9 `normalizeUpdaterStatus` release-channel tests (2026-07-31 evening). |
| Rust (cargo test) | 506 tests (506 pass + 1 ignored) | `cargo test --manifest-path src-tauri/Cargo.toml` | See module breakdown below. +3 wecom signature tests, +1 permission delete test added in Phase 2; +2 v1 mock-transport contract tests added with transport injection (2026-07-31); +18 §8.2 credential migration/strict-mode tests (16 migration incl. startup integration, 2 strict secrets, 4 remote_im config, −3 retired plaintext-fallback tests, −1 net remote_im dedup) (2026-07-31 evening); +10 event-journal recovery tests (assess/close/idempotent marker/corrupt quarantine/write-ahead, AC-1.10) (2026-07-31 evening); +15 AC-8.4 tests (7 ReplayGuard freshness/nonce, 3 engine anti-replay gate, 4 approval TTL/policy, 1 runtime handle accessor) (2026-07-31 evening); +14 AC-1.5 tests (4 subagent-policy clamp, 1 store default, 5 agent_subagents TOML/gate/env, 2 spawn wiring, 1 LiveSession snapshot, 1 host gate) (2026-07-31 evening); +8 AC-1.13 trace-correlation tests (5 mechanism, 3 wiring) (2026-07-31 evening); +8 AC-2.9/AC-7.1 E2E tests (5 AcpClient-level incl. crash injection, 1 engine full-turn, 2 env-gated real-Runtime) (2026-07-31 evening); +8 AC-10.9 update-channel tests (4 UpdateChannel derivation, 1 status DTO release_channel, 3 channel-aware manual path: prerelease semver + per-channel release selection) (2026-07-31 evening). CI-side channel derivation additionally covered by 8 node:test cases (`node --test scripts/update-channel-lib.test.mjs`, not wired into `pnpm test`). |
...
| TC-M.11 | §11 External OMP diagnostics-only + update channels | Update channels: 8 Rust tests (derivation + selection) + 8 node:test CI-derivation cases + `verify-updater-setup.sh` per-channel fetch (2026-07-31) | Manual verification: external OMP read-only; real-device channel smoke (install beta build → pulls beta feed) | Real-platform channel smoke remains manual (rides with AC-10.8/AC-2.12) |
...
| ~~**Single update channel**~~ | AC-10.9 | ~~**Medium**~~ **Resolved 2026-07-31** | Build-time channel baking from version string (design D1): `update_channel.rs` (4 tests) + status DTO (1 test) + channel-aware manual fallback with prerelease semver (3 tests); CI derives channel from tag and publishes per-channel rolling manifests + prerelease flags (`update-channel-lib.mjs`, 8 node:test cases); transitional nightly→`omp-desktop-latest` dual-publish (D5). Live three-feed verification rides with AC-10.8/AC-2.12. |
